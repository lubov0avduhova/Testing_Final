...
public class SearchPage extends BaseMobilePage {
    ...
    public String openResultWithText(String text) {
        ...
        for (int attempt = 0; attempt < Timeouts.SEARCH_RESULT_RETRY_ATTEMPTS; attempt++) {
            List<WebElement> titles = resultTitles();
            ...
            try {
                wait.until(driver -> {
                    List<WebElement> currentTitles = driver.findElements(RESULT_TITLE);
                    return !currentTitles.isEmpty();
                });
            } catch (TimeoutException e) {
                logger.debug("Search results not loaded yet, attempt {}/{}, continuing...", 
                        attempt + 1, Timeouts.SEARCH_RESULT_RETRY_ATTEMPTS);
            }
        }
